  ...
  return (
    <AnimatePresence>
      <nav className="p-7 flex justify-center fixed w-screen z-20">
        <motion.div initial={{opacity: 0, y: -30}} whileInView={{opacity: 1, y:0}} transition={{duration: 0.30}}
          className={`${themeContext?.theme} backdrop-blur-lg bg-opacity-75 text-cusT flex justify-between items-center fixed w-5/6 rounded-full border-2 border-cusT px-5 py-1 mx-auto`}
        >
          <div>
            <h1 className="text-2xl font-bold"><Link to={"/"}>MockEcom</Link></h1>
          </div>
          ...
          {isOpen && (
            <motion.div
              ...
              className={`${themeContext?.theme} backdrop-blur-lg bg-opacity-75 text-cusT md:hidden fixed flex flex-col w-5/6 py-6 px-4 gap-5 -z-10 border-2 rounded-3xl`}
            >
